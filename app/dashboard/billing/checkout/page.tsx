  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col items-center gap-3">
        <div
          className="w-44 h-44 rounded-2xl overflow-hidden flex items-center justify-center"
        ...
        {[
          ...
          >
            ...
            <div className="flex items-center gap-2">
              <span className={`text-sm font-medium ${mono ? 'font-mono' : ''}`} style={{ color: 'var(--text-1)' }} dir="ltr">
                {value}
              </span>
...
function CardPanel({ bill, subCode, onPaid }: { bill: Bill; subCode: string; onPaid: () => Promise<void> }) {
  ...
  return (
    <div className="space-y-4" dir="rtl">
      ...
      <div className="space-y-1.5">
        ...
        <div className="relative">
          ...
          <div className="absolute end-3 top-1/2 -translate-y-1/2 flex gap-1">
            {(isVisa || !cardNum) && (
              <span className="text-xs font-bold px-1.5 py-0.5 rounded" style={{ background: '#1a1f71', color: '#fff' }}>
                VISA
              </span>
            )}
            {(isMC || !cardNum) && (
              <div className="flex -space-x-2 rtl:space-x-reverse">
                ...
                <span className="w-5 h-5 rounded-full bg-yellow-400/80 block" />
              </div>
            )}
          </div>
        </div>
      ...
      {err && (
        <p className="text-xs text-red-400 flex items-center gap-1.5" style={{ fontFamily: 'var(--font-ibm-arabic)' }}>
      ...
      {done ? (
        ...
        >
          {saving ? (
            <><RefreshCw className="w-4 h-4 animate-spin" /> جارٍ المعالجة...</>
          ) : (
            <><Lock className="w-4 h-4" /> دفع {formatIQD(bill.total_iqd)} بالبطاقة</>
          )}
        </button>
      )}
    </div>
...
function CashPanel({ bill, subCode, onPaid }: { bill: Bill; subCode: string; onPaid: () => Promise<void> }) {
  ...
  const isDescValid = desc.includes(subCode); // must reference subscriber ID
    ...
    <div className="space-y-4" dir="rtl">
      ...
      >
        ...
        <p className="text-xs text-amber-400" style={{ fontFamily: 'var(--font-ibm-arabic)' }}>
          هذا السجل للاستخدام الداخلي فقط. يجب أن يتضمن وصف الدفع رمز المشترك تلقائياً.
        </p>
      </div>
      ...
      <div className="space-y-1.5">
        ...
        <input
          ...
          onBlur={(e)  => (e.target.style.borderColor = 'rgba(255,255,255,0.08)')}
        />
        {receipt.trim().length > 0 && receipt.trim().length < 3 && (
          <p className="text-[10px] text-red-400" style={{ fontFamily: 'var(--font-ibm-arabic)' }}>
            رقم الوصل قصير جداً (3 أحرف على الأقل)
          </p>
        )}
      </div>

      {/* Description (must contain sub_code) */}
      <div className="space-y-1.5">
        <div className="flex items-center justify-between">
          ...
          {isDescValid ? (
            <span className="text-[10px] text-emerald-400 flex items-center gap-1">
              <CheckCircle2 className="w-3 h-3" /> يتضمن رمز المشترك
            </span>
          ) : (
            <span className="text-[10px] text-red-400 flex items-center gap-1">
              <AlertTriangle className="w-3 h-3" /> يجب أن يتضمن {subCode}
            </span>
          )}
        </div>
        <textarea
      ...
      >
        ...
        <span className="text-lg font-bold tabular-nums" style={{ color: 'var(--text-1)' }}>
...
// ── Big payment method card ───────────────────────────────────────────────
function MethodCard({
  id, icon: Icon, label, sublabel, accent,
...
// ── Inner checkout (needs useSearchParams) ────────────────────────────────
function CheckoutInner() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const { data: sub } = await supabase
      .from('subscribers')
      .select('id, sub_code, full_name, region_id, amps, active')
      .eq('sub_code', subCode)
      .single();
    ...
    // If no bill this month, auto-create one
    if (!billRes.data && regRes.data) {
      ...
      const { data: newBill } = await supabase
        ...
        .insert({
          ...
          amps: sub.amps,
          price_per_amp: regRes.data.price_per_amp,
          commission: regRes.data.commission,
          total_iqd: total,
          status: 'pending',
        })
        .select('*')
  ...
  useEffect(() => {
    if (!bill) return;
    const chan = supabase
      .channel(`bill-rt-${bill.id}`)
      .on(
        'postgres_changes',
        { event: 'UPDATE', schema: 'public', table: 'bills', filter: `id=eq.${bill.id}` },
        (payload) => {
          setBill((prev) => prev ? { ...prev, ...(payload.new as Partial<Bill>) } : prev);
        }
      )
      .subscribe();
  ...
  if (notFound || !subscriber || !region || !bill) {
    return (
      <div
        className="max-w-md mx-auto mt-20 rounded-3xl p-10 text-center space-y-4"
        style={{ background: 'var(--bg-card)', border: '1px solid var(--border-normal)' }}
        dir="rtl"
      >
        <AlertTriangle className="w-12 h-12 mx-auto text-amber-400" />
        <p className="text-base font-bold" style={{ color: 'var(--text-1)', fontFamily: 'var(--font-ibm-arabic)' }}>
          المشترك غير موجود
        </p>
        <p className="text-sm" style={{ color: 'var(--text-4)', fontFamily: 'var(--font-ibm-arabic)' }}>
      ...
      >
        ...
        <div className="space-y-2">
          {[
            ...
            <div
              key={label}
              className="flex items-center justify-between text-xs px-4 py-2 rounded-lg"
          ...
          >
            <span className="text-xs" style={{ color: 'var(--text-4)', fontFamily: 'var(--font-ibm-arabic)' }}>
              ({subscriber.amps} × {region.price_per_amp.toLocaleString('ar-IQ')}) + {region.commission.toLocaleString('ar-IQ')} =
            </span>
            <span
              ...
              dir="ltr"
            >
              {subscriber.amps} × {region.price_per_amp.toLocaleString()} + {region.commission}
            </span>
          </div>
        </div>
      ...
      {!isPaid && (
        <div className="space-y-3">
          ...
          <MethodCard
            ...
            icon={QrCode}
            label="ZainCash"
            sublabel="دفع إلكتروني عبر محفظة زين كاش"
            accent="#3b82f6"
            selected={method === 'zaincash'}
            onSelect={() => setMethod((m) => m === 'zaincash' ? null : 'zaincash')}
          >
          ...
          <MethodCard
            ...
            icon={Smartphone}
            label="SuperKey"
            sublabel="دفع إلكتروني عبر محفظة SuperKey"
            accent="#a855f7"
            selected={method === 'superkey'}
            onSelect={() => setMethod((m) => m === 'superkey' ? null : 'superkey')}
          >
          ...
          <MethodCard
            ...
            sublabel="Visa / Mastercard — مدفوعات آمنة بتشفير TLS"
            accent="#10b981"
            selected={method === 'card'}
            onSelect={() => setMethod((m) => m === 'card' ? null : 'card')}
          >
          ...
          <MethodCard
            ...
            icon={Banknote}
            label="نقداً"
            sublabel="تسجيل وصل نقدي — لصاحب المولد"
            accent="#f59e0b"
            selected={method === 'cash'}
            onSelect={() => setMethod((m) => m === 'cash' ? null : 'cash')}
          >
